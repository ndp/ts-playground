...
export function defineComponent<Attr extends string, Options extends ComponentOptions<Attr>>(
  ...
  if (options.attrs) {
    for (const attr of options.attrs) {
      const sanitizedKey = santitizeAttr(attr)
      Object.defineProperty(elementClass.prototype, sanitizedKey, {
        get() {
          return this.getAttribute(sanitizedKey) || ''
        },
      })
    }
...
function santitizeAttr(attr: string) {
  return attr.replace(/[*`🗱]/g, '');
}

function requiredAttrs(attrs?: Array<string>): Array<string> {
  return attrs?.filter(a => a.includes('*')).map(a => santitizeAttr(a) ) || []
}
